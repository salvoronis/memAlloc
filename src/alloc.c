...
void * _malloc(size_t query){
	struct mem * pointer;;
	struct mem * new_end;
	if ((pointer = (struct mem *)find_free_mem(query + 2*sizeof(struct mem))) != NULL &&
			pointer->capacity >= query+sizeof(struct mem)) {
		//puts("used");
	...
	if (start) {
		...
		pointer = heap_init(query+sizeof(struct mem) > CHUNK ? query+2*sizeof(struct mem) : CHUNK);
		tmp->next = pointer;
...
void align_mem(){
	...
	while (head->next != NULL) {
		if (head->is_free && head->next->is_free && head+head->capacity == head->next) {
